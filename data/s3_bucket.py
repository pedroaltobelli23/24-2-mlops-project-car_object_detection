...
class S3_Bucket():
    def __init__(self,access_key_id,secret_access_key,region):
        ...
        self.region = region
        self.s3 = boto3.client("s3",aws_access_key_id=self.access_key_id,aws_secret_access_key=self.secret_access_key,region_name=self.region)
    ...
    def create_S3_bucket(self, bucket_name, type="model"):
        try:
            ...
            if type=="model":
                ...
                a = set_key(dotenv_path=find_dotenv(),key_to_set="BUCKET_DATASET",value_to_set=str(bucket_name))
            else:
    ...
    def delete_S3_bucket(self, bucket_name):
        try:
            bucket = boto3.resource("s3",aws_access_key_id=self.access_key_id,aws_secret_access_key=self.secret_access_key,region_name=self.region).Bucket(bucket_name)
    ...
    def add_file_model_S3_bucket(self,bucket_name,file_path,object_name):
        try:
            ...
            logging.info(f"{file_path} saved in the bucket {bucket_name} as {object_name}")
        except Exception as e:
            logging.error(traceback.format_exc())
            return False
        return True
    ...
    def handle_arguments(self,args):
        if any(vars(args).values()):
            if args.file_path:
                res = self.add_file_model_S3_bucket(os.getenv("BUCKET_MODEL"),args.file_path,args.object_name)
                if not res:
            ...
            if args.list_buckets:
                ...
                if not res:
                    logging.error("Error while trying to list all s3 buckets from account")
                    return
...
if __name__=="__main__":
    ...
    parser.add_argument(
        "--bucket_model", type=str, help="Name of the bucket to be created and saved as model bucket."
    )
    
    parser.add_argument(
        "--bucket_dataset", type=str, help="Name of the bucket to be created and saved as dataset bucket."
    )   
    ...
    parser.add_argument(
        "--file_path", type=str, help="Add file to model bucket."
    )
    
    parser.add_argument(
        "--object_name", type=str, help="Object name that will be saved inside the model bucket", default="model.onnx"
    )
    
    
    
    bucket_module = S3_Bucket(os.getenv("AWS_ACCESS_KEY_ID"),os.getenv("AWS_SECRET_ACCESS_KEY"),os.getenv("AWS_REGION"))
